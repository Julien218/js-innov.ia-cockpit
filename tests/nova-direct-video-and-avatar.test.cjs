...
test('Cockpit Elynea uses one canonical JS-Innov.IA 3D companion avatar with legacy NOVA compatibility', () => {
  ...
  assert.match(scope, /const ELYNEA_AVATAR_DATA_URI = 'data:image\/webp;base64,/);
  assert.match(scope, /avatar:\s*ELYNEA_AVATAR_DATA_URI/);
  assert.doesNotMatch(scope, /www\.jsinnovia\.com\/brand\/companion\/companion-avatar-256\.webp/);

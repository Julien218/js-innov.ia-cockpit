...
test('download validates the secured route and successful binary response before starting', async () => {
  ...
  const runtime = { fetch: async function () { 'use strict'; assert.equal(this, undefined); return new Response(new Blob(['pdf']), { headers: { 'Content-Disposition': 'attachment; filename=x.pdf' } }); },
    document: { createElement: () => ({ click: () => { clicked = true; }, remove: () => { removed = true; } }), body: { appendChild() {} } },
    URL: { createObjectURL: () => 'blob:test', revokeObjectURL: () => { revoked = true; } }, setTimeout: function (fn) { 'use strict'; assert.equal(this, undefined); fn(); } };

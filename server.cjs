...
// ── EcranLead / Superagent → Cockpit central ────────────────────
// Authentification serveur-à-serveur par clé dédiée x-ecranlead-key.
// Le Cockpit reste la source de vérité CRM. Un contrat signé convertit
// le lead en Client actif ; aucune commission assurance n'est créée ici.
try {
  const { router: ecranLeadRouter } = require('./server-ecranlead.cjs');
  app.use('/api/integrations/ecranlead', ecranLeadRouter);
  console.log('✅ Route /api/integrations/ecranlead activée (sync CRM central)');
} catch (e) {
  console.warn('⚠️ Route EcranLead indisponible:', e.message);
}

...
async function supabaseRows(path) {
  if (!SUPABASE_SERVICE_KEY) throw new Error('SUPABASE_SERVICE_ROLE_KEY manquante');
...
async function requireInsuranceAccess(req, res, next) {
  ...
  try {
    ...
    const allowed = user?.role === 'superadmin' || email === OLIVIER_EMAIL;
    if (!allowed) return res.status(403).json({ error: 'Accès assurances non autorisé.' });

    req.insuranceUser = { id: user.id, email, role: user.role };
    return next();
  } catch (error) {
...
router.use(async (req, res) => {
  ...
  const target = `${AGENT_URL}/insurance${req.url}`;
  const options = {
    method: req.method,
    headers: {
      'Content-Type': 'application/json',

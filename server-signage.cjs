...
router.post('/manage/media/upload-session', async (req,res) => {
  try {
    ...
    const path=`/Clients/${owner(req)}/Digital Signage/Medias/${Date.now()}-${name}`;
...
router.post('/player/heartbeat', async (req,res)=>{ try{const p=await device(req,'signage_players');if(!p)return res.status(401).json({error:'Player non autorise'});await db(`signage_players?id=eq.${p.id}`,{method:'PATCH',body:JSON.stringify({status:'online',last_seen_at:new Date().toISOString(),app_version:String(req.body.appVersion||''),diagnostics:req.body.diagnostics||{},updated_at:new Date().toISOString()})});const pubs=await db(`signage_publications?select=*&player_id=eq.${p.id}&status=eq.pending&order=created_at.desc&limit=1`);res.json({playerId:p.id,publication:pubs?.[0]||null,nextHeartbeatSeconds:30});}catch(e){res.status(503).json({error:e.message});}});

...
export default function AvatarFactory() {
  ...
  const [form, setForm] = useState({
    ...
    character_id: 'vaincriez-canary', reference_path: '', billing_policy: 'technical_costs_only', preset: 'diagnostic',
  });
  ...
  const create = async event => {
    ...
    try {
      ...
      setNotice({ type: 'success', text: 'Production lancée : référence → 3D → Blender → rig/idle → QA → validation humaine.' });
      await refresh();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-[0.95fr_1fr_1.35fr] gap-5 items-start">
        <form onSubmit={create} className="premium-panel p-5 space-y-4">
          <div><h2 className="font-semibold">Nouvelle production</h2><p className="text-xs text-muted-foreground mt-1">Dépose la référence directement depuis le Cockpit.</p></div>
          <ReferenceUploader previewUrl={previewUrl} uploadedReference={uploadedReference} busy={busy === 'upload'} online={uploaderOnline} onBrowse={() => fileInputRef.current?.click()} onDrop={event => { event.preventDefault(); const file = event.dataTransfer.files?.[0]; if (file) uploadReference(file); }} onClear={clearReference} />
...
function ReferenceUploader({ previewUrl, uploadedReference, busy, online, onBrowse, onDrop, onClear }) {
  return <div onDragOver={event => event.preventDefault()} onDrop={onDrop} className={`rounded-2xl border-2 border-dashed p-4 transition ${uploadedReference ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-primary/20 bg-primary/[0.025]'} ${!online ? 'opacity-60' : ''}`}>{previewUrl ? <div className="space-y-3"><div className="relative rounded-xl overflow-hidden bg-black/5 aspect-square max-h-64"><img src={previewUrl} alt="Référence avatar" className="w-full h-full object-contain" /><button type="button" onClick={onClear} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/70 text-white flex items-center justify-center" aria-label="Retirer l’image"><XCircle className="w-4 h-4" /></button></div><button type="button" disabled={busy || !online} onClick={onBrowse} className="w-full h-10 rounded-xl border border-border text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-50">{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <UploadCloud className="w-4 h-4" />}Remplacer l’image</button></div> : <button type="button" disabled={busy || !online} onClick={onBrowse} className="w-full py-8 flex flex-col items-center justify-center text-center disabled:opacity-50"><div className="w-12 h-12 rounded-2xl gradient-primary text-white shadow-lg shadow-primary/20 flex items-center justify-center mb-3">{busy ? <Loader2 className="w-6 h-6 animate-spin" /> : <ImageIcon className="w-6 h-6" />}</div><p className="text-sm font-semibold">Déposer ou choisir une image</p><p className="text-xs text-muted-foreground mt-1">PNG, JPG/JPEG ou WEBP · max. 12 Mo</p><p className="text-[11px] text-muted-foreground mt-2">Transfert direct vers la station locale.</p></button>}</div>;
}

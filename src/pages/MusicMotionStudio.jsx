...
import Field from '@/components/music-motion/AccessibleField';
...
import { reviewState, requireCurrentReview, requireUnlockedTimeline } from '@/lib/music-motion/review-state';
...
export default function MusicMotionStudio() {
  ...
  async function ingest(files,role,{sceneId,shotId}={}) {
    ...
    for(const file of files){const {asset,blob}=await makeAsset(file,role);Object.assign(asset,await probeMedia(blob));if(current.current.id!==baseId)throw new Error('Le projet a changé pendant l’import.');
      ...
      change(p=>({...p,assets:[...p.assets,asset],...(role==='audio'?{audio:{asset_id:asset.id,name:asset.name,sha256:asset.sha256,duration_seconds:asset.duration_seconds},analysis:null,review:{lyrics:false,storyboard:false},title:p.title==='Nouveau clip'?asset.name.replace(/\.[^.]+$/,''):p.title}:{}),
        ...
        shots:p.shots.map(s=>s.id===shotId&&!s.locked&&!p.scenes.find(v=>v.id===s.scene_id)?.locked?{...s,...(role==='video'?{video_id:asset.id}:{image_id:asset.id})}:s)}));
  ...
  async function queue(input,meta={},paidConsent=false,expectedReview){
    const p=current.current, id=uid('job');
    if(expectedReview!==undefined)requireCurrentReview(p,expectedReview);
    ...
    try{
      ...
      if(expectedReview!==undefined)requireCurrentReview(current.current,expectedReview);
      submitted=true;
      const result=paidConsent?await cloudJson('/jobs',{...wire,consent:{paid:true,external_transfer:true,one_request:true}}):await local.json('/jobs',wire);
      change(v=>v.id===p.id?{...v,jobs:v.jobs.map(j=>j.id===id?{...j,status:result.status,stage:result.stage}:j)}:v);
    }catch(e){change(v=>({...v,jobs:v.jobs.map(j=>j.id===id?{...j,status:!submitted||(e.status&&e.status<500)?'failed':'submission_unknown',error:e.message}:j)}));throw new Error((submitted?'Soumission non confirmée : ':'Soumission annulée avant envoi : ')+e.message+' L’identifiant est conservé ; aucun nouvel essai automatique.');}
  }
  async function analyze(){const p=current.current;if(!p.audio)throw new Error('Importez la chanson complète.');await queue({type:'analyze',audio_id:await uploadLocal(p.audio.asset_id),instrumental},{},false,reviewState(p));}
  async function generate(kind,s,shot){
    ...
    const text=scenePrompt(p,s,shot,kind), meta={scene_id:s.id,shot_id:shot?.id||''}, expectedReview=reviewState(p);
    if(provider==='xai'){
      if(!cloud?.available)throw new Error('L’API payante n’est pas disponible. Vérifiez les moteurs.');
      setPaid({projectId:p.id,sourceId,input:{type:kind,prompt:text,format:p.direction.format,duration_seconds:seconds},meta,expectedReview});return;
    }
    ...
    await queue({type:kind,prompt:text,source_id:await uploadLocal(sourceId),checkpoint,workflow_id:workflow,duration_seconds:seconds,format:p.direction.format},meta,false,expectedReview);
  }
  async function render(animatic){const p=current.current;const errors=renderIssues(p,{animatic});if(errors.length)throw new Error(errors.slice(0,5).join(' '));
    const expectedReview=reviewState(p);
          ...
          try{const result=tracked.provider==='local'?await localRef.current.json('/jobs/'+tracked.id):await cloudJson('/jobs/'+tracked.id);
            ...
            if(result.status==='completed'&&!tracked.applied){
              if(tracked.type==='analyze'){
                ...
                change(p=>({...p,assets:[...p.assets,asset],scenes:p.scenes.map(s=>s.id===tracked.scene_id&&tracked.type==='image'&&!s.locked?{...s,keyframe_id:asset.id}:s),shots:p.shots.map(s=>s.id===tracked.shot_id&&!s.locked&&!p.scenes.find(v=>v.id===s.scene_id)?.locked?{...s,...(tracked.type==='video'?{video_id:asset.id}:{image_id:asset.id})}:s)}));
  ...
  function editShot(id,patch){change(p=>({...p,review:{...p.review,storyboard:false},shots:p.shots.map(s=>s.id===id&&!s.locked&&!p.scenes.find(v=>v.id===s.scene_id)?.locked?{...s,...patch}:s)}),{history:true});}
  async function applyAction(raw,expectedReview){const p=current.current;if(expectedReview!==undefined)requireCurrentReview(p,expectedReview);const a=validateAction(raw,p);
    if(a.type==='storyboard'){
      if(!analysisReady(p))throw new Error('L’analyse complète doit précéder le storyboard.');
      requireUnlockedTimeline(p);
  ...
  async function chat(){const p=current.current;if(!prompt.trim())return;const expectedReview=reviewState(p);const message=prompt;setPrompt('');change(v=>({...v,messages:[...v.messages,{role:'user',text:message}]}));
    const reply=await local.json('/chat',{message,context:{title:p.title,direction:p.direction,duration_seconds:p.audio?.duration_seconds,analysis_complete:analysisReady(p),sections:acoustic?.sections||[],lyrics_reference:p.direction.lyrics_reference||'',lyrics:(p.analysis?.transcription?.segments||[]).map(s=>({start:s.start,end:s.end,text:s.text})),scenes:p.scenes,shots:p.shots.map(s=>({id:s.id,scene_id:s.scene_id,start:s.start,end:s.end,locked:s.locked})),sources:p.sources.map(s=>({name:s.name,text:s.text.slice(0,10000)})),recent_messages:p.messages.slice(-6).map(m=>({role:m.role,text:m.text}))}});
    if(current.current.id!==p.id)return;
    change(v=>({...v,messages:[...v.messages,{role:'assistant',text:reply.message,actions:reply.actions||[],review_state:expectedReview}]}));
  }
  ...
  return <div className="p-4 sm:p-6 max-w-[1600px] mx-auto space-y-5">
    ...
    <section className={panel}><div className="flex flex-wrap justify-between gap-2"><h2 className="font-semibold">Plans modulables · {scene?.label||'sélectionner une scène'}</h2><span className="text-sm">Vraie vidéo disponible : {progress.percent}% · manque {timeLabel(progress.missing)}</span></div><progress className="w-full h-2" max="100" value={progress.percent}/>
      {shots.map((s,i)=><article key={s.id} className="border rounded-lg p-3 space-y-3"><div className="flex flex-wrap justify-between gap-2"><strong className="text-sm">Plan {i+1} · {timeLabel(s.start)} → {timeLabel(s.end)}</strong><button className="text-xs underline" disabled={scene?.locked} onClick={()=>change(p=>({...p,shots:p.shots.map(v=>v.id===s.id?{...v,locked:!v.locked}:v)}))}>{s.locked?'Déverrouiller':'Verrouiller'}</button></div>
        <div className="grid sm:grid-cols-4 gap-2"><Field label="Début"><input className={input} type="number" step="0.01" disabled={s.locked||scene?.locked} value={s.start} onChange={e=>editShot(s.id,{start:Number(e.target.value)})}/></Field><Field label="Fin"><input className={input} type="number" step="0.01" disabled={s.locked||scene?.locked} value={s.end} onChange={e=>editShot(s.id,{end:Number(e.target.value)})}/></Field><Field label="Entrée dans la vidéo"><input className={input} type="number" step="0.01" min="0" disabled={s.locked||scene?.locked} value={s.source_in} onChange={e=>editShot(s.id,{source_in:Number(e.target.value)})}/></Field><Field label="Raccord"><select className={input} disabled={s.locked||scene?.locked} value={s.transition} onChange={e=>editShot(s.id,{transition:e.target.value})}><option value="cut">Coupe franche</option><option value="fade">Fondu par le noir</option></select></Field></div>
        <Field label="Prompt propre à ce plan"><textarea className={input} rows={2} disabled={s.locked||scene?.locked} value={s.prompt} onChange={e=>editShot(s.id,{prompt:e.target.value})}/></Field>
        <div className="grid sm:grid-cols-3 gap-2"><Field label="Image de départ"><select className={input} value={s.image_id||scene?.keyframe_id||''} disabled={s.locked||scene?.locked} onChange={e=>editShot(s.id,{image_id:e.target.value})}><option value="">Aucune image</option>{project.assets.filter(a=>a.mime.startsWith('image/')).map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></Field><Field label="Vidéo retenue"><select className={input} value={s.video_id} disabled={s.locked||scene?.locked} onChange={e=>editShot(s.id,{video_id:e.target.value})}><option value="">Vidéo à produire</option>{project.assets.filter(a=>a.mime.startsWith('video/')&&a.role!=='render').map(a=><option key={a.id} value={a.id}>{a.name} · {timeLabel(a.duration_seconds)}</option>)}</select></Field><Field label="Cadrage"><select className={input} value={s.fit} disabled={s.locked||scene?.locked} onChange={e=>editShot(s.id,{fit:e.target.value})}><option value="contain">Tout conserver</option><option value="cover">Remplir / recadrer</option></select></Field></div>
        <div className="flex gap-2 flex-wrap"><button className={button} disabled={busy||s.locked||scene?.locked} onClick={()=>attempt(()=>generate('video',scene,s))}><Film className="w-4 h-4"/>Animer ce plan</button><Pick label="Importer la vidéo" accept="video/mp4,video/webm,video/quicktime" disabled={busy||s.locked||scene?.locked} onFiles={f=>attempt(()=>ingest(f,'video',{shotId:s.id}))}/><button className={button} onClick={()=>{if(audioRef.current){audioRef.current.currentTime=s.start;setCursor(s.start);}}}>Écouter à ce repère</button></div>
      </article>)}

...
const DOMAINS = [
  {
    name: "jsinnovia.com",
    publicUrl: PLATFORM_SERVICES.publicSite.url,
    dest: "Railway",
    app: "js-innovia-site",
    email: "IONOS (DNS)",
    agent: "JsInnov-Agent",
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-[1500px] mx-auto">
      ...
      <section className="rounded-2xl border border-border bg-card p-4 space-y-3">
        <div>
          <p className="text-sm font-semibold">Services réellement reliés</p>
          <p className="text-xs text-muted-foreground">Le site public, le Cockpit et l’écran géant restent séparés pour éviter toute confusion de domaine ou de dépôt.</p>
        </div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {PLATFORM_SERVICE_LIST.map((service) => (
            <a
              key={service.service}
              href={service.url}
              target="_blank"
            ...
            >
              ...
              <p className="mt-1 text-[11px] text-muted-foreground break-all">{service.domain}</p>
              <p className="mt-2 text-xs">{service.note}</p>
              <p className="mt-2 text-[10px] text-muted-foreground">{service.service} · {service.repository} · {service.branch}</p>
            </a>
          ))}
        </div>

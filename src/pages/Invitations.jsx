  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    {isSuperAdmin ? <form onSubmit={submit} className="rounded-2xl border bg-card p-6">
      ...
      <div className="mt-5 space-y-4 rounded-xl border bg-muted/20 p-4">
        ...
        {groupedCatalog(form.role).map(([group, items]) => <div key={group}><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{group}</p><div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3">{items.map(item => <label key={item.code} className="flex cursor-pointer items-start gap-2 rounded-lg border bg-background p-3 text-sm"><input type="checkbox" disabled={item.code === "dashboard"} checked={form.permissions.includes(item.code)} onChange={() => toggleFormPermission(item.code)} className="mt-0.5 h-4 w-4 accent-[#D4AF37]" /><span><span className="block font-medium">{item.label}{item.code === "dashboard" ? " (obligatoire)" : ""}</span>{item.description && <span className="mt-0.5 block text-xs text-muted-foreground">{item.description}</span>}</span></label>)}</div></div>)}
      </div>
    ...
    {isSuperAdmin && <section className="rounded-2xl border bg-card p-6">
      ...
      <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="py-2 pr-3">Utilisateur</th><th className="py-2 pr-3">Organisation</th><th className="py-2 pr-3">Statut</th><th className="py-2 pr-3">Rôle maximum</th><th className="py-2 text-right">Actions</th></tr></thead>
        <tbody>{users.map(account => { const ownAccount = account.id === user?.id; const accessOpen = openAccess === account.id; return <React.Fragment key={account.id}>
          ...
          {accessOpen && !ownAccount && <tr className="border-b bg-muted/20"><td colSpan={5} className="p-4"><div className="space-y-4"><div><p className="font-semibold">Accès de {account.full_name || account.email}</p><p className="text-xs text-muted-foreground">Cochez uniquement les services auxquels ce client a souscrit. Une case non cochée masque le module et bloque aussi ses API.</p></div>{groupedCatalog(account.role).map(([group, items]) => <div key={group}><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{group}</p><div className="grid gap-2 sm:grid-cols-2 md:grid-cols-4">{items.map(item => <label key={item.code} className="flex cursor-pointer items-start gap-2 rounded-lg border bg-background p-3"><input type="checkbox" disabled={item.code === "dashboard"} checked={(account.permissions || []).includes(item.code)} onChange={() => toggleUserPermission(account.id, item.code)} className="mt-0.5 h-4 w-4 accent-[#D4AF37]" /><span><span className="block font-medium">{item.label}{item.code === "dashboard" ? " (obligatoire)" : ""}</span>{item.description && <span className="mt-0.5 block text-xs text-muted-foreground">{item.description}</span>}</span></label>)}</div></div>)}<div className="flex justify-end"><Button disabled={permissionBusy === account.id} onClick={() => updatePermissions(account)}>{permissionBusy === account.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Enregistrer les accès</Button></div></div></td></tr>}

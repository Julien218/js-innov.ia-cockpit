...
import { Mail, Plus, Copy, Trash2, Clock, CheckCircle2, AlertCircle, Shield, Users, Briefcase, User, Loader2 } from "lucide-react";
...
export default function Invitations() {
  ...
  // Charger les invitations depuis Supabase
  ...
  const handleSend = async (e) => {
    e.preventDefault();
    if (!isSuperAdmin && form.role === "admin") return;
    setSending(true);
    const token = generateToken();
    const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();

    const { data, error } = await supabase.from('cockpit_invitations').insert({
      email: form.email,
      role: form.role,
      token,
  ...
  const revokeInvite = async (id) => {
    await supabase.from('cockpit_invitations').update({ status: 'revoked' }).eq('id', id);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {showForm && (
        <div className="mb-6 p-5 rounded-2xl border-2 border-[#D4AF37]/20 bg-[#001a3d]/5">
          ...
          <form onSubmit={handleSend} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-2">Rôle</label>
                ...
                {ROLE_OPTIONS.map(opt => {
                  ...
                  return (
                    <button key={opt.value} type="button" onClick={() => setForm(p => ({ ...p, role: opt.value }))}
                      className={`p-3 rounded-xl border-2 text-left transition-all ${sel ? "border-current" : "border-gray-100 hover:border-gray-200"}`}
                      style={sel ? { borderColor: colors.badge, backgroundColor: colors.bg + "15" } : {}}>
                      <opt.icon className="w-4 h-4 mb-1" style={{ color: sel ? colors.badge : "#9ca3af" }} />
                      <p className="text-xs font-semibold" style={{ color: sel ? colors.badge : "#374151" }}>{opt.label}</p>
            ...
            {!isSuperAdmin && form.role === "admin" && (
              <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-xs">
                <AlertCircle className="w-4 h-4" /> Seul le Super Admin peut inviter des Admins.
              </div>
            )}
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5">Message (optionnel)</label>
              <textarea value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))}
                placeholder="Bonjour, je vous invite à rejoindre notre espace…" rows={2}
            ...
            <div className="flex gap-3 justify-end">
              <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 text-sm text-gray-500 hover:text-gray-700">Annuler</button>
              <button type="submit" disabled={sending || (!isSuperAdmin && form.role === "admin")}
                className="px-5 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 disabled:opacity-40"
                style={{ backgroundColor: "#001a3d", color: "#D4AF37" }}>
                {sending ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Envoi…</> : <><Mail className="w-3.5 h-3.5" /> Envoyer</>}
              </button>
            </div>
          </form>
        </div>
      ...
      {loading ? (
        <div className="flex justify-center py-12"><Loader2 className="w-6 h-6 text-[#D4AF37] animate-spin" /></div>
      ) : (
        <>
          {pending.length > 0 && (
            <div className="mb-6">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">En attente ({pending.length})</p>
          ...
          {accepted.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Acceptées ({accepted.length})</p>
              <div className="space-y-2">
                {accepted.map(inv => {
                  const colors = ROLE_COLORS[inv.role] || ROLE_COLORS.client;
                  return (
                    <div key={inv.id} className="p-4 bg-green-50/50 rounded-xl border border-green-100 flex items-center gap-3">
                      <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0" />
                      <p className="text-sm font-semibold text-gray-900 flex-1">{inv.email}</p>
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-medium"
                        style={{ backgroundColor: colors.badge + "22", color: colors.badge }}>
                        {ROLE_LABELS[inv.role]}
                      </span>
          ...
          {invitations.length === 0 && (
            <div className="text-center py-16">
              ...
              <button onClick={() => setShowForm(true)} className="mt-4 text-sm font-semibold underline underline-offset-2" style={{ color: "#D4AF37" }}>

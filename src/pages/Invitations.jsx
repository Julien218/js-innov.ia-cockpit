...
export default function Invitations() {
  ...
  const submit = async event => {
    ...
    try {
      const response = await fetch(isSuperAdmin ? "/api/user-access/invite" : "/api/client-signage/invitations", {
        method: "POST",
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-bold"><Shield className="h-6 w-6 text-[#D4AF37]" />Utilisateurs et accès</h1>
        <p className="mt-1 text-sm text-muted-foreground">Invitez un client ou un membre de l’équipe avec uniquement les droits nécessaires.</p>
      </div>

      {error && <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}
      {result && <div className="flex gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-semibold">{result.status === "invited" ? "Invitation envoyée" : result.status === "pending" ? "Une invitation valide existe déjà" : result.status === "role_updated" ? "Rôle mis à jour" : "Ce compte est déjà actif"}</p><p>Aucun mot de passe ni jeton secret n’est affiché dans le Cockpit.</p></div></div>}
      ...
      <form onSubmit={submit} className="rounded-2xl border bg-card p-6">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {isSuperAdmin && <label className="space-y-1 md:col-span-2"><span className="text-sm font-medium">Niveau d’accès</span><select value={form.role} onChange={event => setForm({ ...form, role: event.target.value })} className="h-11 w-full rounded-xl border px-3">{ROLE_OPTIONS.filter(option => option.value !== "superadmin").map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><p className="text-xs text-muted-foreground">Un Administrateur peut gérer l’Écran géant. Un Commercial accède au CRM, aux demandes et aux devis, sans réglages techniques.</p></label>}
        </div>

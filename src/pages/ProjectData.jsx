...
export default function ProjectData() {
  ...
  const loadCollections = async () => {
    ...
    setSelectedKey((current) => current || rows[0]?.key || '');
  };
  ...
  const loadRecords = async (key = selectedKey) => {
    ...
    try {
      ...
      setSelectedCollection(data.collection || null);
    } catch (loadError) {
  ...
  useEffect(() => { if (selectedKey) loadRecords(selectedKey); }, [selectedKey]);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((row) => {
      ...
      return [row.suggested_name, row.reason, row.display_name].some((value) => String(value || '').toLocaleLowerCase('fr').includes(query));
    });
  ...
  const actions = <div className="flex flex-wrap gap-2">
    ...
    {selectedKey && <Button variant="outline" size="sm" asChild><a href={`/api/project-data/${encodeURIComponent(selectedKey)}/export.csv`}><Download /> Export CSV</a></Button>}
  ...
  return <div className="p-4 sm:p-6 lg:p-8 max-w-[1500px] mx-auto">
    ...
    {collections.length === 0 && !loading ? <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center"><Database className="mx-auto mb-3 h-9 w-9 text-muted-foreground" /><h2 className="font-semibold">Aucun tableau partagé</h2><p className="mt-1 text-sm text-muted-foreground">Aucune collecte de données n’est encore rattachée à votre compte.</p></div> : <>
      <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <select value={selectedKey} onChange={(event) => setSelectedKey(event.target.value)} className="h-10 rounded-lg border border-input bg-background px-3 text-sm">
          {collections.map((collection) => <option key={collection.key} value={collection.key}>{collection.label}</option>)}
        </select>
      </div>

      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Metric icon={Database} label="Réponses" value={counts.total} />
        <Metric icon={Filter} label="En attente" value={counts.pending} tone="text-amber-700 bg-amber-100" />
        <Metric icon={CheckCircle2} label="Retenues" value={counts.approved} tone="text-emerald-700 bg-emerald-100" />
        <Metric icon={XCircle} label="Écartées" value={counts.rejected} tone="text-rose-700 bg-rose-100" />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <select value={status} onChange={(event) => setStatus(event.target.value)} className="h-9 rounded-md border border-input bg-background px-3 text-sm"><option value="all">Tous les statuts</option><option value="pending">En attente</option><option value="approved">Retenus</option><option value="rejected">Écartés</option></select>
          </div>
        </div>

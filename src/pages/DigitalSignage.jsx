...
const api = async (path, options = {}) => {
  ...
  if (text && contentType.includes("application/json")) {
    try { body = JSON.parse(text); } catch { throw new Error("RÃ©ponse du cockpit illisible. RÃ©essayez dans quelques secondes."); }
  } else if (text) {
    throw new Error(response.status === 401 ? "Votre session a expirÃ©. Reconnectez-vous." : "Le service du cockpit est momentanÃ©ment indisponible.");
  }
  if (!response.ok) throw new Error(body.error || "OpÃ©ration impossible");
  return body;
};
...
const uploadMedia = (file, onProgress) => new Promise((resolve, reject) => {
  ...
  xhr.upload.onprogress = event => {
    ...
    onProgress({ percent, state: "uploading", label: "Envoi sÃ©curisÃ© vers le cockpitâ€¦" });
  };
  xhr.upload.onload = () => onProgress({ percent: 85, state: "processing", label: "Conversion vidÃ©o compatible MXQ, transfert Dropbox et indexationâ€¦" });
  xhr.onload = () => {
    const contentType = xhr.getResponseHeader("content-type") || "";
    let body = {};
    if (xhr.responseText && contentType.includes("application/json")) {
      try { body = JSON.parse(xhr.responseText); } catch { return reject(new Error("RÃ©ponse du cockpit illisible.")); }
    }
    if (xhr.status < 200 || xhr.status >= 300) return reject(new Error(body.error || `Envoi impossible (HTTP ${xhr.status}).`));
    onProgress({ percent: 100, state: "done", label: "Conversion terminÃ©e. Le mÃ©dia est prÃªt pour le Player." });
    resolve(body);
  };
  xhr.onerror = () => reject(new Error("Connexion interrompue pendant lâ€™envoi."));
  xhr.ontimeout = () => reject(new Error("Lâ€™envoi a pris trop de temps. RÃ©essayez avec une connexion stable."));
  xhr.send(file);
});
...
export default function DigitalSignage() {
  ...
  }, "Player crÃ©Ã©. Le jeton dâ€™installation est conservÃ© dans cette session uniquement.");

  const rotatePlayerToken = () => run(async () => {
    if (!player) throw new Error("CrÃ©ez dâ€™abord le Player");
    const result = await api(`/manage/players/${player.id}/rotate-token`, { method: "POST", body: "{}" });
    setEnrollmentToken(result.enrollmentToken);
  }, "Nouveau jeton gÃ©nÃ©rÃ©. Lâ€™ancien jeton est maintenant dÃ©sactivÃ©.");

  const upload = file => run(async () => {
    setTransfer({ name: file.name, size: file.size, percent: 0, state: "uploading", label: "PrÃ©paration du fichierâ€¦" });
    try {
  ...
  }, "MÃ©dia converti, envoyÃ© et prÃªt pour la diffusion.");

  const createPlaylist = () => run(async () => {
    if (!media[0]) throw new Error("Ajoutez dâ€™abord un mÃ©dia");
  ...
  }, "Playlist crÃ©Ã©e avec le mÃ©dia le plus rÃ©cent.");

  const publish = () => run(async () => {
    if (!player) throw new Error("CrÃ©ez dâ€™abord le Player");
    if (!playlists[0]) throw new Error("CrÃ©ez dâ€™abord une playlist");
    await api("/manage/publications", { method: "POST", body: JSON.stringify({ playerId: player.id, playlistId: playlists[0].id }) });
  }, "Diffusion programmÃ©e. Elle sera rÃ©cupÃ©rÃ©e au prochain heartbeat du Player.");

  return <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
    <PageHeader title="Ã‰cran gÃ©ant" subtitle="Pilotage du Player HDMI reliÃ© au contrÃ´leur Colorlight X2M." />
    ...
    {enrollmentToken && <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 space-y-2"><p className="font-semibold">Jeton dâ€™association â€” affichÃ© une seule fois</p><p className="text-xs text-muted-foreground">Saisissez exactement ce jeton sur la MXQ, puis effacez-le de toute note temporaire.</p><div className="flex gap-2"><input readOnly value={enrollmentToken} className="flex-1 rounded-lg border bg-background px-3 py-2 font-mono text-sm"/><button onClick={() => navigator.clipboard.writeText(enrollmentToken)} className="rounded-lg border px-4 py-2 text-sm">Copier</button></div></div>}
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      <StatusCard icon={Wifi} label="Player" value={player ? (player.status === "online" ? "ConnectÃ©" : "Hors ligne") : "Ã€ crÃ©er"} detail={player?.last_seen_at ? `Dernier signal : ${new Date(player.last_seen_at).toLocaleString("fr-BE")}` : "Heartbeat sÃ©curisÃ© requis"} />
      <StatusCard icon={MonitorPlay} label="ContrÃ´leur LED" value="Colorlight X2M" detail="Sortie Player en HDMI" />
      <StatusCard icon={HardDrive} label="MÃ©diathÃ¨que" value={`${media.length} mÃ©dia(s)`} detail={media[0]?.status === "ready" ? "Dernier mÃ©dia prÃªt pour le Player" : "Conversion du dernier mÃ©dia requise"} />
      <StatusCard icon={RotateCcw} label="Publications" value={`${publications.length}`} detail={latestPublication?.status || "Aucune"} />
    </div>
    {latestPublication?.status === "failed" && <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-700"><p className="font-semibold">La derniÃ¨re diffusion a Ã©chouÃ© sur le Player.</p><p className="mt-1">{latestPublication.error || "Le Player nâ€™a pas pu lire le mÃ©dia."}</p></div>}
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 rounded-2xl border border-border bg-card p-5">
        <h2 className="text-sm font-semibold">Ã‰tat rÃ©el</h2>
        <div className="mt-4 grid grid-cols-3 gap-3 text-center"><div className="rounded-xl bg-muted/30 p-4"><b>{media.length}</b><p className="text-xs text-muted-foreground">MÃ©dias</p></div><div className="rounded-xl bg-muted/30 p-4"><b>{playlists.length}</b><p className="text-xs text-muted-foreground">Playlists</p></div><div className="rounded-xl bg-muted/30 p-4"><b>{publications.length}</b><p className="text-xs text-muted-foreground">Diffusions</p></div></div>
        {!player && <button disabled={busy} onClick={createPlayer} className="mt-4 rounded-xl bg-primary text-primary-foreground px-4 py-2 text-sm">CrÃ©er le Player Olivier</button>}
        {player && <div className="mt-4 space-y-2"><button disabled={busy} onClick={rotatePlayerToken} className="rounded-xl border border-border px-4 py-2 text-sm disabled:opacity-50">GÃ©nÃ©rer un nouveau jeton dâ€™association</button><p className="text-xs text-muted-foreground">Ã€ utiliser aprÃ¨s la rÃ©installation du Player. La gÃ©nÃ©ration dÃ©sactive immÃ©diatement lâ€™ancien jeton.</p></div>}
      </div>
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3"><h2 className="text-sm font-semibold">Actions</h2>
        <a href="/api/player-download/android" download className="w-full rounded-xl border border-border px-4 py-3 text-sm font-medium flex items-center justify-center gap-2"><Download className="w-4 h-4" /> TÃ©lÃ©charger le Player Android 0.3</a>
        ...
        <button disabled={busy || isLoading} onClick={() => fileInput.current?.click()} className="w-full rounded-xl bg-primary text-primary-foreground px-4 py-3 text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50"><Upload className="w-4 h-4" /> Ajouter un mÃ©dia</button>
        <button disabled={busy || !media.length} onClick={createPlaylist} className="w-full rounded-xl border border-border px-4 py-3 text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50"><ListVideo className="w-4 h-4" /> CrÃ©er une playlist</button>

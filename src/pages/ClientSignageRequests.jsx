...
import { CheckCircle2, Clock3, FileVideo2, Image, Loader2, MessageSquareText, UploadCloud, XCircle } from "lucide-react";
...
export default function ClientSignageRequests() {
  const [data, setData] = React.useState({ requests: [], assets: [], reviews: [] });
  ...
  const decide = async (review, decision) => {
    ...
    try {
      const result = await api(`/reviews/${review.id}/decision`, {
        method: "POST",
        body: JSON.stringify({ decision, comment: comments[review.id] || "" }),
      });
      setMessage(result.publication?.id
        ? `Vidéo validée et diffusion programmée (preuve ${result.publication.id}).`
        : decision === "approved"
          ? "Vidéo validée. La diffusion reste suivie dans le Cockpit."
  ...
  const preview = async review => {
    try {
      const response = await fetch(`/api/signage/manage/media/${review.media_id}/download`, { credentials: "same-origin" });
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {pendingReviews.length > 0 && <section className="space-y-3">
        ...
        {pendingReviews.map(review => {
          ...
          const item = mediaById.get(review.media_id);
          return <article key={review.id} className="rounded-2xl border border-amber-200 bg-amber-50/40 p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="font-semibold">{request?.title || "Création vidéo"} — version {review.version}</p><p className="text-sm text-muted-foreground">{item?.name || "Fichier final"}</p></div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button variant="outline" onClick={() => preview(review)}><FileVideo2 className="mr-2 h-4 w-4" />Voir la vidéo</Button>
              ...
              <Button disabled={busy} onClick={() => decide(review, "approved")}><CheckCircle2 className="mr-2 h-4 w-4" />Valider</Button>
            </div>
          </article>;
        })}
      </section>}

...
export default function VideoSurveillance() {
  ...
  return <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
    ...
    {isAdmin && <div className="rounded-2xl border p-5 space-y-4"><h2 className="text-sm font-semibold">Installation de la passerelle</h2>{!gateway ? <button disabled={busy || !enabled} onClick={createGateway} className="rounded-xl bg-primary text-primary-foreground px-4 py-2 text-sm">Créer la passerelle du client</button> : <div className="grid gap-3 md:grid-cols-5"><input value={cameraForm.name} onChange={e => setCameraForm(current => ({ ...current, name: e.target.value }))} className="rounded-xl border bg-background px-3 py-2 text-sm" placeholder="Nom"/><input value={cameraForm.model} onChange={e => setCameraForm(current => ({ ...current, model: e.target.value }))} className="rounded-xl border bg-background px-3 py-2 text-sm" placeholder="Modèle"/><input value={cameraForm.localStreamKey} onChange={e => setCameraForm(current => ({ ...current, localStreamKey: e.target.value }))} className="rounded-xl border bg-background px-3 py-2 text-sm" placeholder="Clé locale"/><input type="number" min="1" max="90" value={cameraForm.retentionDays} onChange={e => setCameraForm(current => ({ ...current, retentionDays: Number(e.target.value) }))} className="rounded-xl border bg-background px-3 py-2 text-sm" aria-label="Conservation en jours"/><button disabled={busy} onClick={createCamera} className="rounded-xl border px-4 py-2 text-sm flex items-center justify-center gap-2"><Plus className="w-4 h-4"/>Ajouter</button></div>}</div>}

...
import {
  ...
  PlayCircle, GalleryHorizontalEnd, Send, Server, Gauge, Workflow, Factory, WalletCards,
} from "lucide-react";
...
const useValidationsBadge = () => {
  const { data = [] } = useQuery({ queryKey: ["validations"], queryFn: () => base44.entities.Validation.list(), staleTime: 30000 });
...
const useEmailBadge = () => {
  ...
  React.useEffect(() => {
    const poll = () => fetch('/api/emails?limit=1', { credentials: 'same-origin' }).then(r => r.ok ? r.json() : { unread: 0 }).then(d => setUnread(d.unread || 0)).catch(() => {});
    poll();
...
const useDemandeBadge = () => {
  const { data = [] } = useQuery({ queryKey: ["demandes"], queryFn: () => base44.entities.Demande.list(), staleTime: 30000 });
...
const useAgentLocalStatus = () => {
  ...
  React.useEffect(() => {
    const check = () => fetch('http://127.0.0.1:8787/health', { signal: AbortSignal.timeout(3000) }).then(r => setStatus(r.ok ? 'online' : 'offline')).catch(() => setStatus('offline'));
    check();
...
const ROLE_ICONS = { superadmin: Crown, admin: Shield, collaborateur: Briefcase, client: User };
...
const allNavGroups = [
  ...
  { label: "CRM", minRole: "collaborateur", items: [
  ...
  { label: "Écosystème", minRole: "admin", items: [
...
export default function Sidebar({ mobileOpen = false, onCloseMobile }) {
  ...
  const insuranceAllowed = role === 'superadmin' || String(user?.email || '').toLowerCase() === OLIVIER_EMAIL;
  ...
  const navGroups = allNavGroups.map(group => ({ ...group, items: group.items.filter(item => item.insuranceOnly ? insuranceAllowed : canAccess(getPath(item))) })).filter(group => group.items.length > 0);
  const handleNavClick = () => { if (mobileOpen && onCloseMobile) onCloseMobile(); };

  return <>
    {mobileOpen && <div className="md:hidden fixed inset-0 bg-black/50 z-40 animate-in fade-in duration-200" onClick={onCloseMobile} />}
    <aside className={cn(

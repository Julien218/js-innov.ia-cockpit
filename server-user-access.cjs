...
router.patch('/:id', async (req, res) => {
  try {
    ...
    if (target.id === req.user.id && role !== 'superadmin') {
      return res.status(409).json({ error: 'Vous ne pouvez pas retirer votre propre accès Super administrateur' });
    }
    const updated = await authDb(`cockpit_users?id=eq.${encodeURIComponent(target.id)}`, {
      method: 'PATCH',

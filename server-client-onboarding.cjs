...
const inviteHash = value => crypto.createHash('sha256').update(value).digest('hex');
...
async function ensureClientInvitation({ email, fullName, organisation, role = 'client' }) {
  ...
  try {
    ...
    await mailTransport().sendMail({
      from: `JS-Innov.IA <${from}>`, to: normalized, subject: 'Activez votre accès Cockpit JS-Innov.IA',
